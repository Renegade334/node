...
class QuicSession {
  ...
  async sendDatagram(datagram) {
    ...
    if (typeof datagram === 'string') {
      // Avoid using `Buffer.from()`, since this may use the buffer pool,
      // which is untransferable.
      const utf8Length = Buffer.byteLength(datagram, 'utf8');
      const buffer = Buffer.allocUnsafeSlow(utf8Length);
      assert.strictEqual(buffer.write(datagram, 'utf8'), utf8Length);
      datagram = buffer;
    } else {
