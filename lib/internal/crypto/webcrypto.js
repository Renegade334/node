...
async function digest(algorithm, data) {
  ...
  return toSafePromise(
    ReflectApply(asyncDigest, this, [algorithm, data]));
}
...
async function generateKey(
  ...
  switch (algorithm.name) {
    ...
    case 'ChaCha20-Poly1305':
      resultType = 'CryptoKey';
      result = await toSafePromise(
        require('internal/crypto/chacha20_poly1305')
  ...
  switch (format) {
    case 'spki': {
      if (key[kKeyType] === 'public') {
        result = await toSafePromise(exportKeySpki(key));
      }
      break;
    }
    case 'pkcs8': {
      if (key[kKeyType] === 'private') {
        result = await toSafePromise(exportKeyPkcs8(key));
      }
      break;
    }
    case 'jwk': {
      result = await toSafePromise(exportKeyJWK(key));
      break;
    }
    case 'raw-secret': {
      if (key[kKeyType] === 'secret') {
        result = await toSafePromise(exportKeyRawSecret(key, format));
      }
      break;
    }
    case 'raw-public': {
      if (key[kKeyType] === 'public') {
        result = await toSafePromise(exportKeyRawPublic(key, format));
      }
      break;
    }
    case 'raw-seed': {
      if (key[kKeyType] === 'private') {
        result = await toSafePromise(exportKeyRawSeed(key));
      }
      break;
    }
    case 'raw': {
...
async function wrapKey(format, key, wrappingKey, algorithm) {
  ...
  let keyData = await toSafePromise(
    ReflectApply(exportKey, this, [format, key]));
...
async function sign(algorithm, key, data) {
  ...
  return toSafePromise(signVerify(algorithm, key, data));
}
...
async function verify(algorithm, key, signature, data) {
  ...
  return toSafePromise(signVerify(algorithm, key, data, signature));
}
...
async function encrypt(algorithm, key, data) {
  ...
  algorithm = normalizeAlgorithm(algorithm, 'encrypt');
  return toSafePromise(
    cipherOrWrap(kWebCryptoCipherEncrypt, algorithm, key, data, 'encrypt'));
}
...
async function decrypt(algorithm, key, data) {
  ...
  return toSafePromise(
    cipherOrWrap(kWebCryptoCipherDecrypt, algorithm, key, data, 'decrypt'));
}
...
async function encapsulateKey(encapsulationAlgorithm, encapsulationKey, sharedKeyAlgorithm, extractable, usages) {
  ...
  switch (encapsulationKey[kAlgorithm].name) {
    ...
    case 'ML-KEM-768':
    case 'ML-KEM-1024':
      encapsulateBits = await toSafePromise(
        require('internal/crypto/ml_kem')
          .mlKemEncapsulate(encapsulationKey));
      break;
    default:
...
async function decapsulateKey(
  ...
  switch (decapsulationKey[kAlgorithm].name) {
    ...
    case 'ML-KEM-768':
    case 'ML-KEM-1024':
      decapsulatedBits = await toSafePromise(
        require('internal/crypto/ml_kem')
          .mlKemDecapsulate(decapsulationKey, ciphertext));
      break;
    default:
