...
function generateKey(algorithm, extractable, keyUsages) {
  try {
    ...
    return PromisePrototypeThen(result, onResolveKey);

    function onResolveKey(result) {
      if (
        (resultType === 'CryptoKey' &&
...
function deriveKey(
  ...
  try {
    ...
    return PromisePrototypeThen(bits, onResolveBits);

    function onResolveBits(bits) {
      return ReflectApply(
        importKeySync,
        this,
...
function exportKeySpki(key) {
  try {
    switch (key[kAlgorithm].name) {
      ...
      case 'ML-DSA-87':
        ...
        return PromiseResolve(
          require('internal/crypto/ml_dsa')
    ...
    // Note: mlDsaExportKey and mlKemExportKey can throw synchronously,
    // so we need the catch block here.
    return PromiseReject(err);
  }
}
...
async function exportKeyRawSeed(key) {
  // Note: this function does not await or return any Promises,
  // so it's safe to use an async function to wrap the return value.
  switch (key[kAlgorithm].name) {
    case 'ML-DSA-44':
      // Fall through
    case 'ML-DSA-65':
...
function exportKey(format, key) {
  try {
    ...
    return PromisePrototypeThen(result ?? PromiseResolve(undefined), onResolveKey);
...
function wrapKey(format, key, wrappingKey, algorithm) {
  try {
    ...
    const keyData = ReflectApply(exportKey, this, [format, key]);

    // `cipherOrWrap()` returns a Promise, but passing a Promise directly to
    // the `PromisePrototypeThen` resolver will wrap this Promise in a new
    // outer Promise, which involves a call to the mutable
    // `%Promise.prototype%.then`.
    // Instead, we create a new outer Promise ourselves, then manually wrap the
    // Promise from `cipherOrWrap()` by calling `PromisePrototypeThen` within
    // the `onResolveKeyData` callback.
    const { promise, resolve, reject } = PromiseWithResolvers();
    PromisePrototypeThen(keyData, onResolveKeyData, reject);
    return promise;

    function onResolveKeyData(keyData) {
      if (format === 'jwk') {
      ...
      PromisePrototypeThen(result, resolve, reject);
    }
  } catch (err) {
    return PromiseReject(err);
  }
...
function unwrapKey(
  ...
  try {
    ...
    return PromisePrototypeThen(keyData, onResolveKeyData);
...
function encapsulateKey(
  ...
  try {
    ...
    return PromisePrototypeThen(encapsulateBits, onResolveEncapsulateBits);

    function onResolveEncapsulateBits(encapsulateBits) {
...
function decapsulateKey(
  ...
  try {
    ...
    return PromisePrototypeThen(decapsulatedBits, onResolveDecapsulatedBits);

    function onResolveDecapsulatedBits(decapsulatedBits) {
      return ReflectApply(
        importKeySync,
        this,
