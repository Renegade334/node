...
function hmacGenerateKey(algorithm, extractable, keyUsages) {
  try {
    ...
    const key = generateKey('hmac', { length });

    return PromisePrototypeThen(key, onResolveKey, onRejectKey);

    function onResolveKey(key) {
...
function kmacGenerateKey(algorithm, extractable, keyUsages) {
  try {
    ...
    const keyData = randomBytes(length / 8);

    return PromisePrototypeThen(keyData, onResolveKeyData, onRejectKeyData);
