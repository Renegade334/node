...
function pbkdf2DeriveBits(algorithm, baseKey, length) {
  try {
    ...
    const result = pbkdf2Promise(
      baseKey[kKeyObject].export(),
      salt,
      iterations,
      length / 8,
      normalizeHashName(hash.name));

    return PromisePrototypeThen(result, onResolveResult, onRejectResult);

    function onResolveResult(result) {
      return TypedArrayPrototypeGetBuffer(result);
    }

    function onRejectResult(err) {
      throw lazyDOMException(
