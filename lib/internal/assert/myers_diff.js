...
function selectTypedArrayConstructor(max) {
  if (max < 2 ** 7) {
    return Int8Array;
  }
  if (max < 2 ** 15) {
  ...
  throw new Error('too big, oopsie');
}
...
function myersDiff(actual, expected, checkCommaDisparity = false) {
  ...
  const TypedArrayConstructor = selectTypedArrayConstructor(max);
  const v = new TypedArrayConstructor(2 * max + 1);
  const trace = [];

  for (let diffLevel = 0; diffLevel <= max; diffLevel++) {
    ArrayPrototypePush(trace, new TypedArrayConstructor(v)); // Clone the current state of `v`
