...
function testFastMethods() {
  histogram.record(1);
  assert.throws(() => histogram.record(-1));
...
if (common.isDebug) {
  ...
  assert.strictEqual(getV8FastApiCallCount('histogram.record.ok'), 1);
  assert.strictEqual(getV8FastApiCallCount('histogram.record.error'), 1);
