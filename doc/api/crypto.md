...
changes:
  - version: REPLACEME
    pr-url: REPLACEME
    description: Calling `randomBytes()` without a callback parameter is
                 deprecated in favour of `randomBytesSync()`.
  - version: v18.0.0
    pr-url: https://github.com/nodejs/node/pull/41678
    description: Passing an invalid callback to the `callback` argument
                 now throws `ERR_INVALID_ARG_TYPE` instead of
...
Bytes are generated asynchronously, and the `callback` function is invoked with
two arguments: `err` and `buf`. If an error occurs, `err` will be an `Error`
...
sufficient entropy available. This should normally never take longer than a few
...
still low on entropy.

This API uses libuv's threadpool, which can have surprising and
negative performance implications for some applications; see the
[`UV_THREADPOOL_SIZE`][] documentation for more information.

Calls to `crypto.randomBytes()` are carried out in a single threadpool request.
To minimize threadpool task length variation, partition large `randomBytes()`
requests when doing so as part of fulfilling a client request.

### `crypto.randomBytes(size)`

> Stability: 0 - Deprecated: Use [`crypto.randomBytesSync()`][] instead.

* `size` {number}
* Returns: {Buffer}

Calling `crypto.randomBytes()` without a `callback` parameter is equivalent to
calling [`crypto.randomBytesSync()`][].
...
Synchronous version of [`crypto.randomBytes()`][].
...
[`crypto.randomBytesSync()`]: #cryptorandombytessyncsize
